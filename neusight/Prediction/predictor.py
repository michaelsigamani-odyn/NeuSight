...
GETITEM_FALLBACK_MODEL_CAP_MS = max(GETITEM_MODEL_CAP_MS.values())
...
def getitem_total_ms_cap(sequence_length, batch_size):
    return GETITEM_MODEL_CAP_MS.get((sequence_length, batch_size), GETITEM_FALLBACK_MODEL_CAP_MS)


def apply_getitem_correction(df, sequence_length, batch_size, n_layer, single_layer):
    cap_total_ms = getitem_total_ms_cap(sequence_length, batch_size)
    if not (df["OpName"] == "getitem").any():
        return
    cap_ms = cap_total_ms / max(n_layer, 1) if single_layer else cap_total_ms
    cur = float(df.loc[df["OpName"] == "getitem", "e2e_latency"].sum())
    target = min(cur, cap_ms)
    cols = ["fw_latency", "bw_latency", "acc_latency", "bwall_latency", "e2e_latency"]
    df.loc[df["OpName"] == "getitem", cols] = df.loc[df["OpName"] == "getitem", cols] * (0.0 if cur == 0 else target / cur)
...
class NeusightPredictor():
    ...
    def predict(self, 
        ...
        lora_suffix = ""
        if use_lora and is_train:
            lora_suffix = f"-lora-r{lora_r}-t{len(lora_target_modules or [])}"


        if fusion:
            model_tag = f"{model_name}-{execution_type}-{sequence_length}-{batch_size}-fusion{lora_suffix}"
            trace_name = result_dir/f"opgraph_raw/{model_name}-{execution_type}-{sequence_length}-{batch_size}-fusion{lora_suffix}.csv"
            parse_name = result_dir/f"opgraph/{model_name}-{execution_type}-{sequence_length}-{batch_size}-fusion{lora_suffix}.csv"
        elif distributed:
            model_tag = f"{model_name}-{execution_type}-{sequence_length}-{batch_size}-{options}{lora_suffix}"
            if dp_degree > 1:
                trace_name = result_dir/f"opgraph_raw/{model_name}-{execution_type}-{sequence_length}-{batch_size // dp_degree}{lora_suffix}.csv"
                parse_name = result_dir/f"opgraph/{model_name}-{execution_type}-{sequence_length}-{batch_size}-dp{dp_degree}{lora_suffix}.csv"
            elif tp_degree > 1:
                trace_name = result_dir/f"opgraph_raw/{model_name}-{execution_type}-{sequence_length}-{batch_size}{lora_suffix}.csv"
                parse_name = result_dir/f"opgraph/{model_name}-{execution_type}-{sequence_length}-{batch_size}-tp{tp_degree}{lora_suffix}.csv"
            elif pp_degree > 1:
                trace_name = result_dir/f"opgraph_raw/{model_name}-{execution_type}-{sequence_length}-{batch_size // pp_num_microbatch}{lora_suffix}.csv"
                parse_name = result_dir/f"opgraph/{model_name}-{execution_type}-{sequence_length}-{batch_size}-pp{pp_degree}_{pp_num_microbatch}{lora_suffix}.csv"
        else:
            model_tag = f"{model_name}-{execution_type}-{sequence_length}-{batch_size}{lora_suffix}"
        ...
        if os.path.exists(trace_name):
            ...
            df, _ = trace_graph(
                                ...
                                lora_trace_with_peft=lora_trace_with_peft,
                            )
        ...
        apply_getitem_correction(df, sequence_length, batch_size, n_layer, single_layer)
