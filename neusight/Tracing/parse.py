...
def parse_ops(name, input_shapes, output_shape, meta, vocab_size, contiguous, is_train, linear_weight_trainable=True, lora_rank=0):
    name = name.lower()
    ...
    if "norm_weight" in name:
        ...
        if linear_weight_trainable:
            bw_ops.append(("Linear", (O, B, I)))
            acc_ops.append(("VECadd", [1, O*I]))
            acc_ops.append(("VECadd", [1, O]))
        if lora_rank > 0:
            fw_ops.append(("Linear", (B, I, lora_rank)))
            fw_ops.append(("Linear", (B, lora_rank, O)))
            bw_ops.append(("Linear", (B, O, lora_rank)))
            bw_ops.append(("Linear", (B, lora_rank, I)))
            bw_ops.append(("Linear", (lora_rank, B, O)))
            bw_ops.append(("Linear", (I, B, lora_rank)))
            acc_ops.append(("VECadd", [1, lora_rank*I]))
            acc_ops.append(("VECadd", [1, lora_rank*O]))
...
def parse_trace(
    ...
    has_linear_trainable = "linear_weight_trainable" in df.columns
    has_lora_rank = "lora_rank" in df.columns
    df[["OpName", "FwOps", "BwOps", "AccOps"]] = df.apply(lambda x: parse_ops(x.iloc[columns.index("Name")], x.iloc[columns.index("input_shapes")], x.iloc[columns.index("output_shape")], x.iloc[columns.index("meta")], vocab_size, x.iloc[columns.index("input_contiguous")], is_train, x.iloc[columns.index("linear_weight_trainable")] if has_linear_trainable else True, x.iloc[columns.index("lora_rank")] if has_lora_rank else 0), axis=1)
    ...
    df = df.rename(columns={
                    ...
                    "linear_weight_trainable" : "LinearWeightTrainable",
                    "lora_rank" : "LoraRank"
                })
    
    if "LinearWeightTrainable" not in df.columns:
        df["LinearWeightTrainable"] = True
    if "LoraRank" not in df.columns:
        df["LoraRank"] = 0
    columns = ["Name", "OpName", "FwOps", "BwOps", "AccOps", "Prev", "Next", "InputShapes", "OutputShape", "LinearWeightTrainable", "LoraRank"]
    df = df[columns]
                ...
                if len(row["FwOps"]) > 0:
                    if row["FwOps"][0][0] == 'VECln':
                        ...
                        if row["LoraRank"] > 0:
                            ...
                            r = row["LoraRank"]
                            grad_size = r * I + r * O
                        elif row["LinearWeightTrainable"]:
                            B, I, O = row["FwOps"][0][1]
                            grad_size = I * O + O
                        else:
                        ...
                        new_row = {
                                    ...
                                    'OpName': 'allreduce',
                                    'FwOps': [],
                                    'BwOps': [['ALLREDUCE_ASYNC', (grad_size,)]],
                                    'AccOps': [],
                                    'InputShapes': [],
                                    'OutputShape': [],
                                    'Prev' : set(),
                                    'Next' : set(),
                                    'LinearWeightTrainable': row["LinearWeightTrainable"],
