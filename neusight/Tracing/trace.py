...
def _is_lora_target(module_path, lora_target_modules):
    if not module_path:
        return False
    return any(t in module_path for t in lora_target_modules)

def _freeze_for_lora(model):
    for param in model.parameters():
        param.requires_grad = False
    return model

def _module_path_from_meta(meta):
    stack = meta.get("nn_module_stack") if isinstance(meta, dict) else None
    if not stack:
        return ""
    return list(stack.keys())[-1]

def _is_linear_weight_trainable(module_path, modules):
    if module_path not in modules:
        return True
    module = modules[module_path]
    if not hasattr(module, "weight"):
        return True
    return bool(module.weight.requires_grad)

def _lora_rank_for_node(module_path, modules, use_lora, lora_r, lora_target_modules, lora_trace_with_peft):
    if lora_trace_with_peft:
        return 0
    if not use_lora or not _is_lora_target(module_path, lora_target_modules):
        return 0
    if module_path not in modules:
        return 0
    return lora_r if hasattr(modules[module_path], "weight") else 0

def get_model(model_config_path, is_train, device, fusion, use_lora=False, lora_r=16, lora_alpha=32, lora_dropout=0.0, lora_target_modules=None, lora_trace_with_peft=False):
    ...
    try:
        if is_train:
            if "bert" in model_name.lower():
                ...
                model = AutoModelForCausalLM.from_config(config, attn_implementation="eager")
            if use_lora and lora_trace_with_peft:
                from peft import LoraConfig, TaskType, get_peft_model
                lora_cfg = LoraConfig(task_type=TaskType.CAUSAL_LM, r=lora_r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, target_modules=lora_target_modules, bias="none")
                model = get_peft_model(model, lora_cfg).get_base_model()
            elif use_lora:
                model = _freeze_for_lora(model)
            model = model.train()
        else:
            if "bert" in model_name.lower():
...
def trace_fx_graph(batch_size, sequence_length, model_config_path, is_train, bench, single_layer, fusion, use_lora=False, lora_r=16, lora_alpha=32, lora_dropout=0.0, lora_target_modules=None, lora_trace_with_peft=False):
    ...
    try:
        if bench:
            ...
            device = "cpu"
        model, n_layer = get_model(model_config_path=tmp_fname, is_train=is_train, device=device, fusion=False, use_lora=use_lora, lora_r=lora_r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, lora_target_modules=lora_target_modules, lora_trace_with_peft=lora_trace_with_peft) # trace non-fused model instead
    ...
    modules = dict(graphmodule.named_modules())
    node_dict = []
    for n in graphmodule.graph.nodes:
        nd = n.__dict__.copy()
        nd["Name"] = n.name
        nd["meta"] = n.meta
        module_path = _module_path_from_meta(n.meta)
        nd["module_path"] = module_path
        nd["linear_weight_trainable"] = _is_linear_weight_trainable(module_path=module_path, modules=modules)
        nd["lora_rank"] = _lora_rank_for_node(module_path=module_path, modules=modules, use_lora=use_lora and is_train, lora_r=lora_r, lora_target_modules=lora_target_modules, lora_trace_with_peft=lora_trace_with_peft)
...
def trace_graph(
    ...
    if bench:
        ...
        df = trace_fx_graph(batch_size=batch_size, sequence_length=sequence_length, model_config_path=model_config_path, is_train=is_train, bench=True, single_layer=single_layer, fusion=fusion, use_lora=use_lora, lora_r=lora_r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, lora_target_modules=lora_target_modules, lora_trace_with_peft=lora_trace_with_peft)
        gc.collect()
        torch.cuda.empty_cache()
        ...
        # trace operator graph
        df = trace_fx_graph(batch_size=batch_size, sequence_length=sequence_length, model_config_path=model_config_path, is_train=is_train, bench=False, single_layer=single_layer, fusion=fusion, use_lora=use_lora, lora_r=lora_r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, lora_target_modules=lora_target_modules, lora_trace_with_peft=lora_trace_with_peft)
